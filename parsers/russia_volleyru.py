...
class RussiaVolleyRuParser(BaseParser):
    ...
    def _fetch_single_phase(self, url: str):
        ...
        for row in rows:
            ...
            if not total_span:
                continue
            total = total_span.get_text(strip=True)
            match = re.search(r'(\d+):(\d+)', total)
            if not match:
                continue
            hs, aws = map(int, match.groups())
            stats[home]['sets_won'] += hs
            stats[home]['sets_lost'] += aws
            stats[away]['sets_won'] += aws
            stats[away]['sets_lost'] += hs

            rounds_span = row.find('span', class_='s-table__rounds-score')
            if rounds_span:
                pairs = re.findall(r'(\d+):(\d+)', rounds_span.get_text())
        ...
        # Если очки не найдены в детальной таблице, берём их из матричной (data-balls)
        if not any(v['points_won'] > 0 for v in stats.values()):
            matrix = soup.find('table', class_='s-table')
            if matrix and 's-table--round' not in matrix.get('class', []):
                points = {}
                ...
                for row in tbody.find_all('tr'):
                    ...
                    balls = cells[-1].get('data-balls')
                    if balls and ':' in balls:
                        pw, pl = map(int, balls.split(':'))
                        points[team] = (pw, pl)
                for team, (pw, pl) in points.items():
                    if team in stats:
