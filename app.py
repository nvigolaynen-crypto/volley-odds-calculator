...
# ==================== ПАРСЕРЫ ТАБЛИЦ (CSV, EXCEL, ТЕКСТ) ====================
...
with st.sidebar:
    ...
    with st.expander("➕ Новая таблица"):
        ...
        if upload_method == "Текстовый ввод":
            ...
            text_data = st.text_area("Введите данные", height=200, key="text_data")
            if st.button("Создать таблицу", key="create_text_table"):
            ...
            uploaded_file = st.file_uploader("CSV или Excel", type=['csv', 'xlsx'], key="uploaded_file")
            if uploaded_file and st.button("Создать таблицу", key="create_file_table"):
                try:
    ...
    if st.session_state.user_tables:
        ...
        with st.expander("Обновить таблицу"):
            upd_name = st.selectbox("Выберите таблицу", list(st.session_state.user_tables.keys()), key="upd_name")
            upd_method = st.radio("Способ", ["Текстовый ввод","CSV/Excel"], key="upd_method")
            if upd_method == "Текстовый ввод":
                upd_text = st.text_area("Новые данные", height=150, key="upd_text")
                if st.button("Обновить", key="upd_text_button"):
                ...
                upd_file = st.file_uploader("Файл", type=['csv','xlsx'], key="upd_file")
                if upd_file and st.button("Обновить", key="upd_file_button"):
                    try:
...
if st.session_state.active_source == "auto":
    with st.form("auto_form"):
        url = st.text_input("Введите URL страницы с результатами", placeholder="https://...dataproject.com/CompetitionStandings.aspx?ID=127", key="url_input")
        combine = False
        if "dataproject.com" in url:
            combine = st.checkbox("Складывать все этапы (только для Data Project)", value=False, key="combine_phases")
            if combine:
    ...
    if st.session_state.df_teams is not None and not st.session_state.df_teams.empty:
        with st.expander("⚙️ Указать количество сыгранных матчей"):
            ...
            use_manual_matches = st.checkbox("Задать количество матчей вручную", key="manual_matches_checkbox")
            if use_manual_matches:
                matches_value = st.number_input("Количество матчей для всех команд", min_value=1, value=30, step=1, key="manual_matches_value")
                if st.button("Применить ко всем командам", key="apply_manual_matches"):
    ...
    with col1:
        ...
        home_name = st.text_input("Название", key="manual_home_name")
        h_sv = st.number_input("Сеты выиграно", min_value=0, key="manual_home_sv")
        h_sp = st.number_input("Сеты проиграно", min_value=0, key="manual_home_sp")
        h_bv = st.number_input("Очки набрано", min_value=0, key="manual_home_bv")
        h_bp = st.number_input("Очки пропущено", min_value=0, key="manual_home_bp")
        h_m = st.number_input("Матчей", min_value=1, value=30, key="manual_home_matches")
    with col2:
        st.markdown("**Гостевая**")
        away_name = st.text_input("Название", key="manual_away_name")
        a_sv = st.number_input("Сеты выиграно", min_value=0, key="manual_away_sv")
        a_sp = st.number_input("Сеты проиграно", min_value=0, key="manual_away_sp")
        a_bv = st.number_input("Очки набрано", min_value=0, key="manual_away_bv")
        a_bp = st.number_input("Очки пропущено", min_value=0, key="manual_away_bp")
        a_m = st.number_input("Матчей", min_value=1, value=30, key="manual_away_matches")
    if st.button("Сохранить пару", key="save_manual_pair"):
    ...
    if st.session_state.user_tables:
        selected = st.selectbox("Выберите таблицу", list(st.session_state.user_tables.keys()), key="user_table_select")
        if st.button("Активировать", key="activate_user_table"):
...
if st.session_state.df_teams is not None and not st.session_state.df_teams.empty:
    if 'Команда' not in st.session_state.df_teams.columns:
        ...
        gender = st.radio("Категория", ["Мужчины", "Женщины"], index=0 if st.session_state.detected_gender == "Мужчины" else 1, key="gender_radio")
        neutral_field = st.checkbox("Нейтральное поле", key="neutral_field")
        match_format = st.radio("Формат матча", ["до 3 побед (best-of-5)", "до 2 побед (best-of-3)"], index=0, key="match_format")
        col1, col2 = st.columns(2)
        with col1:
        ...
        if current_h2h:
            ...
            if st.button("Очистить историю этой пары", key="clear_h2h"):
        ...
        with st.form(key="add_h2h_form"):
            ...
            with col_a:
                hh = st.selectbox("Хозяева", all_teams, key="h2h_home_select")
                ha = st.selectbox("Гости", all_teams, key="h2h_away_select")
            with col_b:
                sets_h2h = st.text_input("Счёт по сетам (опционально)", placeholder="3:1", key="h2h_sets")
                pts_input = st.text_input("Счёт по очкам (обязательно в формате 75:41 или просто разница)", placeholder="75:41 или 34", key="h2h_pts")
                date_h2h = st.text_input("Дата", placeholder="01.01.2026", key="h2h_date")
        ...
        use_h2h = st.checkbox("Учитывать личные встречи (включено – усреднение, выключено – вычитание из статистики)", value=True, key="use_h2h")

        if st.button("Рассчитать котировки", key="calc_button"):
            if home == away:
                ...
                # Очищенная статистика (вычитаем только встречи с full_score == True)
                ...
                if use_h2h:
                    ...
                    if clean_raw is not None and avg_h2h is not None:
                        ...
                        st.info(f"📊 **Режим: Без учёта личных встреч**\n"
                                f"Вычтено встреч с полным счётом: {subtracted}\n"
                                f"Фора после вычитания: {clean_raw:.2f}\n"
                                f"Средняя фора личек: {avg_h2h:.2f}\n"
                                f"**Итоговая сырая фора** = ({clean_raw:.2f} + {avg_h2h:.2f}) / 2 = **{final_raw:.2f}**")
                        if subtracted == 0:
                            st.warning("Вычитание не производилось – нет личных встреч с полным счётом по очкам.")
