    ...
    if 'Команда' not in st.session_state.df_teams.columns:
        ...
        if st.button("Рассчитать котировки", key="calc"):
            if home == away:
                ...
                # Очищенная статистика (вычитаем личные встречи, где есть фора)
                ...
                for match in current_h2h:
                    if match['Фора по очкам'] is not None:
                        # Если есть полный счёт через двоеточие – вычитаем очки
                        ...
                        # В любом случае вычитаем матчи (уменьшаем количество)
                        h_matches_clean -= 1
                        a_matches_clean -= 1
                        subtracted += 1
                if subtracted > 0:
                ...
                if use_h2h:
                    ...
                    if clean_raw is not None and avg_h2h is not None:
                        ...
                        if subtracted == 0:
                            st.warning("Внимание: вычитание личных встреч не производилось (не хватает полных счетов по очкам или нет данных).")
